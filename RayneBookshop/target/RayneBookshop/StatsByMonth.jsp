...
<script>
        $.ajax({
            type: "GET",//请求方式
            url: "getallorders.action",//地址，就是action请求路径
            ...
            success: function(msg) {//返回的参数就是 action里面所有的有get和set方法的参数
                ...
                if(dataObj.length == 0){
                	...
                	var temPrice = 0;
                	show += "<h1>这是您的历史订单</h1>"
                	show += "<p>*************"+(temYear+1900)+"年"+(temMonth+1)+"月*************";
                	for (var i = 0; i < dataObj.length; i++) {
                		if(temYear == dataObj[i].date.year && temMonth == dataObj[i].date.month)
                        {
                        	...
                        	if( i == dataObj.length - 1){
                        		show +="<p>当天销售总数为："+temAmount;
                            	show +="<p>当天销售总额为："+temPrice;
                        	}
                        }
                		else{
                        	...
                        	show +="<p>当月销售总数为："+temAmount;
                        	show +="<p>当月销售总额为："+temPrice;
                        	temAmount = 0;
                        	temPrice = 0;
                        	show += "<p>*************"+(temYear+1900)+"年"+(temMonth+1)+"月*************";
                        	show+='<li class="item"> <img src=\"'+dataObj[i].img 
